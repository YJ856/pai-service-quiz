...
export class QuizQueryAdapter implements QuizQueryPort {
  ...
  async findLastScheduledDateYmd(
  ...
  async findFamilyParentsCompleted(
    ...
    const cursorWhere = paginationCursor
      ? {
          OR: [
            {
              publishDate: {
                lt: ymdToUtcDate(paginationCursor.publishDateYmd),
              },
            }, // lt: lessThan
            {
              AND: [
  ...
  async findFamilyParentsScheduled(
    ...
    const cursorWhere = paginationCursor
      ? {
          OR: [
            {
              publishDate: {
                gt: ymdToUtcDate(paginationCursor.publishDateYmd),
              },
            },
            {
              AND: [
  ...
  async findChildrenCompleted(
    ...
    const resultRecords = await this.prisma.assignment.findMany({
      ...
      orderBy: [{ quiz: { publishDate: 'desc' } }, { quizId: 'desc' }],
      take: limit + 1,
      select: {
        quizId: true,
        quiz: {
