...
import type {
  ParentsCompletedResponseResult,
  ParentsCompletedItemDto,
} from '../port/in/result/parents-completed-quiz-result.dto';
...
@Injectable()
export class ListParentsCompletedService
  implements ListParentsCompletedUseCase
{
  constructor(
  ...
  async execute(
    command: ParentsCompletedQuizCommand,
  ): Promise<ParentsCompletedResponseResult> {
    const { limit, cursor } = command;
    const rawCursor = decodeCompositeCursor(cursor ?? null);
    const paginationCursor = rawCursor
      ? {
          publishDateYmd: rawCursor.publishDateYmd,
          quizId: BigInt(rawCursor.quizId),
        }
      : undefined;
    ...
    const parentMap: Record<number, ParentProfileSummary> = Object.fromEntries(
      (family.parents ?? []).map((parent) => [parent.profileId, parent]),
    );
    const childMap: Record<number, ChildProfileSummary> = Object.fromEntries(
      (family.children ?? []).map((child) => [child.profileId, child]),
    );

    // 가족 내 모든 부모 ID
    const familyParentIds = (family.parents ?? []).map(
      (parent) => parent.profileId,
    );
    const familyChildIds = (family.children ?? []).map(
    ...
    const { items: familyRows, hasNext } =
      await this.repo.findFamilyParentsCompleted({
    ...
    const items: ParentsCompletedItemDto[] = familyRows.map((row) => {
      ...
      const children = familyChildIds.map((childId) => {
        ...
        const found = solvedList.find(
          (assignment) => assignment.childProfileId === childId,
        );
        return {
