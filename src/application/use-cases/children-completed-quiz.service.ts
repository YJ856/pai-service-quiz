 ...
 */
@Injectable()
export class ListChildrenCompletedService
  implements ListChildrenCompletedUseCase
{
  constructor(
  ...
  async execute(
    command: ChildrenCompletedQuizCommand,
  ): Promise<ChildrenCompletedResponseResult> {
    ...
    const { items: familyRows, hasNext } =
      await this.repo.findChildrenCompleted({
        childProfileId,
