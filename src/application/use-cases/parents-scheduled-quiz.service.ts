...
import type {
  ParentsScheduledResponseResult,
  ParentsScheduledItemDto,
} from '../port/in/result/parents-scheduled-quiz-result.dto';
...
@Injectable()
export class ListParentsScheduledService
  implements ListParentsScheduledUseCase
{
  constructor(
  ...
  async execute(
    command: ParentsScheduledQuizCommand,
  ): Promise<ParentsScheduledResponseResult> {
    ...
    const parentMap: Record<number, ParentProfileSummary> = Object.fromEntries(
      (parents ?? []).map((parent) => [parent.profileId, parent]),
    );
    ...
    const { items: familyRows, hasNext } =
      await this.repo.findFamilyParentsScheduled({
