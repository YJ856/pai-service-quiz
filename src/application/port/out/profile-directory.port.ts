 ...
 * - profileId: number, avatarMediaId: bigint
 */
...
export interface ProfileDirectoryPort {
   ...
   * - 없으면 null
   */
  getParentProfile(parentProfileId: number): Promise<ParentProfileSummary | null>;

  /**
   * 자녀 프로필 다건 조회 (배치)
   * - 입력 ids에 대한 결과만 반환(없는 id는 누락 가능)
   * - 성능을 위해 Map/Record 형태로 반환 (키는 number의 string 표현)
   */
  getChildProfiles(childProfileIds: number[]): Promise<Record<string, ChildProfileSummary>>;
}
